...
def candlestick_dashboard():
    try:
        ...
        try:
            from model_helper import get_latest_symbol_prediction
            ai_pred = get_latest_symbol_prediction(symbol)
            m6.metric(
                "AI Prediction",
                ai_pred["prediction_signal"]
            )
        except Exception:
            m6.metric("AI Prediction", "Evaluating...")
